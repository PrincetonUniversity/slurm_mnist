...
TensorFlow will run all possible operations on the GPU by default. However, if you request more than one GPU in your Slurm script TensorFlow will only use one and ignore the others unless your actively make the appropriate changes to your TensorFlow script. This is covered in the next section. A second mistake occurs when someone accidentally Conda install `tensorflow` instead of `tensorflow-gpu`. The former cannot use the GPU.
